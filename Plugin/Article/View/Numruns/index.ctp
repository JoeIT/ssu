...
<div class="container">
	<h2>
		<?php echo __('Mantenimiento Turnos: (Inicio)'); ?>
	</h2>
	...
	<div class="row-fluid show-grid">
		<?php if ($this->Acl->check('Numruns','add','Article') == true){?>
		<div class="span12" style="text-align: right;">
			<button class="btn btn-success" type="button"
				onclick="showAddNumrunPage();">
				<i class="icon-plus icon-white"></i>
				<?php echo __('Mantenimiento de Turno'); ?>
			</button>
		</div>
		<?php }?>
	</div>
	<?php echo $this->Form->create('Numrun', array('action' => 'index','class'=>' form-signin form-horizontal')); ?>
	<div class="row-fluid show-grid">
		<div class="span12">
	...
	<div class="row-fluid show-grid">
		<div class="span12">
			...
			<table class="table table-bordered table-hover list table-condensed table-striped">
                            <thead>
                                <tr>
                                    <th style="width: 30px;"><?php echo $this->Paginator->sort('NUM_RUNID','Id'); ?>
                                    </th>
                                    <th style="text-align: center;"><?php echo $this->Paginator->sort('NAME','Detalle'); ?>
                                    </th>
                                    <th style="text-align: center; width:190px;"><?php echo $this->Paginator->sort('STARTDATE','Fecha Inicio'); ?>
                                    </th>
                                    <th style="text-align: center; width:190px;"><?php echo $this->Paginator->sort('ENDDATE','Fecha Final'); ?>
                                    </th>
                                    <?php if ($this->Acl->check('Numruns','view','Article') == true || $this->Acl->check('Numruns','edit','Article') == true || $this->Acl->check('Numruns','delete','Article') == true){?>
				...
				<?php
	foreach ($numruns as $numrun): ?>
				<tr>
                                    <td style="text-align: center;"><?php echo h($numrun['Numrun']['NUM_RUNID']); ?>&nbsp;</td>
                                    <td><?php echo h($numrun['Numrun']['NAME']); ?>&nbsp;</td>
                                    <td style="text-align: center;"><?php echo $numrun['Numrun']['STARTDATE']; ?></td>
                                    <td style="text-align: center;"><?php echo $numrun['Numrun']['ENDDATE']; ?></td>
                                    <?php if ($this->Acl->check('Numruns','view','Article') == true || $this->Acl->check('Numruns','edit','Article') == true || $this->Acl->check('Numruns','delete','Article') == true){?>
                                    <td style="text-align: center;">
                                            <?php echo $this->Acl->link(__('Ver'), array('action' => 'view', $numrun['Numrun']['NUM_RUNID']),array('class'=>'btn btn-mini')); ?>
                                            <?php echo $this->Acl->link(__('Editar'), array('action' => 'edit', $numrun['Numrun']['NUM_RUNID']),array('class'=>'btn btn-mini btn-primary')); ?>
                                            <?php echo $this->Acl->link(__('Eliminar'), array('action' => 'delete', $numrun['Numrun']['NUM_RUNID']),array('class'=>'btn btn-mini btn-danger','onclick' =>'delNumrun("'.h($numrun['Numrun']['NUM_RUNID']).'","'.h($numrun['Numrun']['NAME']).'");return false;')); ?>
                                    </td>
                                    <?php } ?>
				</tr>
				<?php endforeach; ?>
...
function showAddNumrunPage() {
	window.location = "<?php echo Router::url(array('plugin' => 'article','controller' => 'Numruns','action' => 'add')); ?>";
}
function delNumrun(numrun_id, name) {
    $.sModal({
        ...
        buttons: [{
            ...
            click: function(id, data) {
                $.post('<?php echo Router::url(array('plugin' => 'article','controller' => 'Numruns','action' => 'delete')); ?>/' + numrun_id, {}, function(o) {
	                    $('#container').load('<?php echo Router::url(array('plugin' => 'article','controller' => 'Numruns','action' => 'index')); ?>', function() {
                        $.sModal('close', id);
