...
<div class="container">
    <div style="float:left;"> Movimientos Contrato</div>
    <div style="float:right;">
    <?php 
    echo $this->Html->link($this->Html->image('pdf_.png', array('width' => '30', 'heigth' => '30', 'alt' =>'Pdf', 'title' => 'Reporte pdf')),array('action' => 'contratopdf',$fi,$ff,$id),array('escape'=>false,'class'=>'btn btn-mini'));
    echo $this->Html->link($this->Html->image('excel_.png', array('width' => '30', 'heigth' => '30', 'alt' =>'Excel', 'title' => 'Reporte excel')),array('action' => 'contratoexcel',$fi,$ff,$id),array('escape'=>false,'class'=>'btn btn-mini'));
    ?></div>
    <table style="float: left;" class="table-bordered table-hover list table-condensed table-striped">
        ...
        foreach($userinfos AS $userinfo){
            ...
            if(empty($tempsch[0])){
                ...
                $comision=$deiles->query("SELECT dbo.comisiont('".$ffi."','".$f011."',".$userinfo['Userinfo']['USERID'].") AS comision;");
                ...
                $fech1=date ("Y-m-d",strtotime ('+1 day',strtotime($fff)));
                $result=$deiles->query("SELECT dbo.horarios01('".$ffi."','".$fech1."',".$userinfo['Userinfo']['USERID'].") AS retraso;");
                $sumat=$result[0][0]['retraso'];
            }
        ?>
                <tr>
