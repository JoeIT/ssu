	...
	public function login() {
		...
		if ($this->request->is('post')) {
			if ($this->Auth->login()) {
				...
				$error = __('Su Correo o Contraseña son incorretos.');
			}
		}
					...
					if (!empty($user['User']['user_code'])){
						...
						if ($user['User']['user_code'] == sha1('reset_password'.$user['User']['user_email'].$aryCode[1]).'-'.$aryCode[1]){
							if (time() - $aryCode[1] < 24*60*60){
								...
								$errors['user_code'][] = __('Ha caducado Su código de restablecimiento de contraseña');
							}
						}
					}
				}else{
					$errors['user_code'][] =  __('No existe el código de restablecimiento de contraseña');
				}
