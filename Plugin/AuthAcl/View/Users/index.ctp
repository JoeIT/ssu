	...
	<div class="row-fluid show-grid">
		<div class="span12">
			<div class="input-append">
				<?php echo $this->Form->input('filter',array('div' => false,'label'=>false,'placeholder'=>"Buscar Usuarios",'class'=>'input-xlarge')); ?>
			...
			<table class="table table-bordered table-hover list table-condensed table-striped">
				<thead>
					<tr>
						...
						<th style="text-align: center; width: 250px;"><?php echo $this->Paginator->sort('user_email',__('Correo de Usuario')); ?>
						</th>
						<th style="text-align: center;"><?php echo $this->Paginator->sort('user_name',__('Nombre Completo')); ?>
						</th>
						<th style="text-align: center; width: 150px;">Grupos</th>
						<th style="text-align: center; width: 60px;"><?php echo $this->Paginator->sort('user_status',__('Estado')); ?>
						</th>
						<th style="text-align: center; width: 150px;"><?php echo $this->Paginator->sort('created',__('Creado')); ?>
						</th>
						...
						<th style="text-align: center; width: 160px;"><?php echo __('Acciones'); ?>
						</th>
						<?php } ?>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<?php if ($this->Acl->check('Users','view','AuthAcl') == true || $this->Acl->check('Users','edit','AuthAcl') == true || $this->Acl->check('Users','delete','AuthAcl') == true){?>
						<td style="text-align: center;">
							<?php echo $this->Acl->link(__('Ver'), array('action' => 'view', $user['User']['id']),array('class'=>'btn btn-mini')); ?>
							<?php echo $this->Acl->link(__('Editar'), array('action' => 'edit', $user['User']['id']),array('class'=>'btn btn-mini btn-info')); ?>
							<?php  if ($auth_user['id'] != $user['User']['id']){?> 
								<?php echo $this->Acl->link(__('Eliminar'), array('action' => 'delete', $user['User']['id']),array('class'=>'btn btn-mini btn-danger','onclick' =>'delUser("'.h($user['User']['id']).'","'.h($user['User']['user_email']).'");return false;')); ?>
							<?php }else{ ?> 
								<?php if ($this->Acl->check('Users','delete','AuthAcl') == true){?>
									<?php echo $this->Acl->link(__('Eliminar'), array(),array('class'=>'btn btn-mini btn-danger disabled','onclick'=>'return false;')); ?>
								<?php } ?> 
							<?php }?>
						</td>
						<?php } ?>
...
<script>
	...
	function delUser(user_id, email) {
	    $.sModal({
	        ...
	        content: '<?php echo __('Estas seguro para Eliminar'); ?>  <b>' + email + '</b>?',
	        animate: 'fadeDown',
	        buttons: [{
